...
### Structured-hours hardening (deferred from eng review, low severity)

Latent, not live — no current venue triggers either. Captured so they aren't lost.

- **`hoursNote` hidden when `hours` is null** — in `SpaceCard.vue` the `hoursNote` `<p>` is nested inside `v-if="hoursText"`, so a venue with `hours: null` **and** a `hoursNote` shows nothing on the card, while `issueUrl.ts` still surfaces the note in the prefilled issue. The two consumers disagree. Fix: render the note independently of `hoursText` (show the Hours block when `hoursText || space.hoursNote`). Doesn't bite today (the 7 `null`-hours venues have no note). Start: `src/components/SpaceCard.vue:150`.
- **Loose time format accepted by `clockToMinutes`** — the regex allows a 1-digit hour (`"9:00"`) and `"24:00"` as an *open*. Two effects: (1) `formatHours` groups by raw string, so `"9:00"` and `"09:00"` are the same schedule but render as separate day-groups; (2) `open: "24:00"` spills a bogus full next-day in `buildSchedule`. Fix options: normalize `formatHours` grouping by minutes (fixes the cosmetic split regardless of input), and/or reject `open === "24:00"` in `validateWeeklyHours`. The migrated data is already clean (all 2-digit, no open `24:00`); this only hardens against future hand-edits / LLM output. Start: `src/utils/hoursBasic.ts:41` (`clockToMinutes`), `:126` (`formatHours`).

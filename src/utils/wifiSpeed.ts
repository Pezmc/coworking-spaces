...
// down >100 → fast, 25–100 → medium, <25 → slow; null/garbage → null (unknown).
export function deriveWifiSpeed(mbps: IWifiSpeedMbps | null): WifiSpeed | null {
  if (!mbps || !Number.isFinite(mbps.down) || mbps.down <= 0) return null
  if (mbps.down > FAST_THRESHOLD_MBPS) return 'fast'
  if (mbps.down >= MEDIUM_FLOOR_MBPS) return 'medium'
  return 'slow'
}

// Effective bucket: derived from a measurement when present, else the stored value (null = unknown).
